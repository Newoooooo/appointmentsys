...
const FilterDropdown = ({ options, activeFilter, onSelect, align = "right" }) => {
    const isActive = String(activeFilter || '').toLowerCase() !== 'all';
        ...
        <DropdownMenu
            ...
            trigger={
                <button className={clsx(
                    ...
                    isActive
                        ? 'border-[#F26389] text-[#F26389] bg-[#F26389]/5'
                        : 'border-[#e6e4e6] dark:border-white text-[#b1b1b1]'
                )}>
                    <Filter size={14} />
                    {isActive && (
                        <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[#F26389] rounded-full" />
                    )}
                </button>
            }
        >
            <div className="py-2">
                {options.map((opt) => (
                    ...
                    >
                        <span className={activeFilter === opt ? "text-[#F26389]" : "opacity-60"}>
                            {opt}
                        </span>
                        {activeFilter === opt && <Check size={14} className="text-[#F26389]" />}
                    </button>
                ))}
            </div>
        </DropdownMenu>
