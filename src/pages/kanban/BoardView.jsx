...
export const BoardView = ({ columns, tasks, scrollRef, onScroll, onDragStart, onDropOnColumn, draggingTaskId, onTaskClick }) => {
            ...
            <div className="flex gap-6 min-h-full pb-8">
                {columns.map((col) => (
                    <div
                        key={col.name}
                        className={clsx(
                            'flex flex-col w-[280px] md:flex-1 shrink-0 group/col rounded-2xl transition-colors',
                            dragOverCol === col.name ? 'bg-[#F26389]/5 ring-2 ring-[#F26389]/20' : ''
                        )}
                        onDragOver={(e) => {
                            e.preventDefault();
                    ...
                    >
                        ...
                        <div className="relative space-y-3 pl-4 border-l border-[#f4f2f4] dark:border-white/5 flex-1">
