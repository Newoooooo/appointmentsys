...
import { InquiryService } from "../../api/services.js";
...
const Inquiries = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = InquiryService.subscribeToInquiries((inquiries) => {
            setInquiriesData(inquiries);
            if (inquiries.length > 0 && !activeInq) {
                setActiveInq(inquiries[0]);
            }
        ...
        <div className="bg-[#fdfcfc] dark:bg-[#080808] text-[#2f3035] dark:text-[#fdfcfc] p-4 lg:p-6 flex flex-col overflow-hidden">
            <header className="flex items-center justify-between gap-3 mb-8 pb-6 border-b border-[#f4f2f4] dark:border-white/5 shrink-0 relative z-20">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button className="h-9 w-9 bg-[#F26389] text-white rounded-xl flex items-center justify-center shadow-lg shadow-[#F26389]/20 shrink-0">
            ...
            <div className="flex-1 flex gap-6 overflow-hidden relative">
                ...
                <div className={clsx(
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-3 pl-4 pr-1 border-l border-[#f4f2f4] dark:border-white/5 pb-4">
                        {isLoadingInquiries ? (
                            <div className="py-4 text-center text-[#b1b1b1]">
                                <p className="text-[10px] font-bold uppercase">Loading inquiries...</p>
                            </div>
                        ) : inquiriesData.length === 0 ? (
                            <div className="py-4 text-center text-[#b1b1b1]">
                                <p className="text-[10px] font-bold uppercase">No inquiries</p>
                            </div>
                        ) : (
